...
def _main(file_name, _dict):
	
	#Uncomment if vocab is not available.
	vocab = get_vocab(file_name)
	save_dict(vocab, _dict)
	print len(vocab)
	f = 5
	vocab_wf = vocab_with_frequency(vocab, f)
	save_dict(vocab_wf, _dict + '_wf5')
	print len(vocab_wf)

if __name__ == '__main__':

	if len(sys.argv) != 3:
		print 'Usage: python ', sys.argv[0], 'corpus_file', 'save_dict_to'
		exit()
